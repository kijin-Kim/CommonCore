...
void Renderer::Init(int width, int height)
{
	...
	projection_ = glm::ortho(-static_cast<float>(width) / 2.0f, static_cast<float>(width) / 2.0f, -static_cast<float>(height) / 2.0f, static_cast<float>(height) / 2.0f, -1.0f, 1.0f);
	...
	glBufferData(GL_ARRAY_BUFFER, ShapeUtility::UnitCircleVertices.size() * sizeof(glm::vec2), ShapeUtility::UnitCircleVertices.data(), GL_STATIC_DRAW);
	...
	glBufferData(GL_ARRAY_BUFFER, ShapeUtility::SquareVertices.size() * sizeof(glm::vec2), ShapeUtility::SquareVertices.data(), GL_STATIC_DRAW);
	glEnableVertexAttribArray(0);
	glVertexAttribPointer(0, 2, GL_FLOAT, GL_FALSE, 2 * sizeof(float), nullptr);
...
void Renderer::Draw(unsigned int vao, GLenum mode, GLsizei vertexCount, const glm::mat4& transform, const glm::vec4& color)
{
